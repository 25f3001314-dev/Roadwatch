...
public class ComplaintService {
    ...
    public Complaint updateComplaint(Long id, ComplaintUpdateRequest request) {
        ...
        if (request.getDeptResponse() != null && !request.getDeptResponse().isBlank()) {
            complaint.setDeptResponse(request.getDeptResponse());
        }
        if (request.getAssignedOfficer() != null && !request.getAssignedOfficer().isBlank()) {
            complaint.setAssignedOfficer(request.getAssignedOfficer());
        }
        if (request.getExpectedCompletion() != null && !request.getExpectedCompletion().isBlank()) {
            try {
                complaint.setExpectedCompletion(
                        LocalDateTime.parse(request.getExpectedCompletion()));
            } catch (Exception e) {
                logger.warn("Invalid expectedCompletion format: {}", request.getExpectedCompletion());
            }
        }
        if (request.getResolutionImageUrl() != null && !request.getResolutionImageUrl().isBlank()) {
            complaint.setResolutionImageUrl(request.getResolutionImageUrl());
        }

        // Auto-set resolvedAt when status becomes RESOLVED
        String currentStatus = complaint.getStatus();
        if ("RESOLVED".equals(currentStatus) && complaint.getResolvedAt() == null) {
        ...
        // ── Phase 1: Audit trail + FCM notification ──────────────────────
        ...
        if (newStatus != null && !newStatus.equals(oldStatus)) {
            ...
            history.setNotes(request.getAdminNotes());
            statusHistoryRepository.save(history);

            // Fire push notification to complaint owner
            try {
                notificationService.onStatusChange(saved, oldStatus, newStatus);
            } catch (Exception e) {
                // Notification failure must never break the admin update flow
                logger.error("Failed to send notification for complaint {}: {}", id, e.getMessage());
            }
        }
